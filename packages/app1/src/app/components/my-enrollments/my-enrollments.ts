...
} from '@monorepo-angular-20/utils-common';
...
import {
  ...
  TableColumn,
  TableComponent,
} from '@monorepo-angular-20/ui-shared';
...
export class MyEnrollmentsComponent implements OnInit, OnDestroy {
  ...
  tableActions: TableAction[] = [
    {
      ...
      onClick: (enrollment: EnrollmentWithDetails) =>
        this.cancelEnrollment(enrollment),
    },
  ];
  ...
  loadEnrollments(): void {
    ...
    this.apiService
      .getStudentEnrollmentsWithDetails(currentStudentId)
      .subscribe({
        next: (enrollments) => {
        ...
        error: (error) => {
          ...
          this.error =
            'No se pudieron cargar tus inscripciones. Verifica que el servidor esté ejecutándose.';
  ...
  cancelEnrollment(enrollment: EnrollmentWithDetails): void {
    ...
    if (
      confirm(
        `¿Estás seguro de que quieres cancelar tu inscripción en "${courseName}"?`
      )
    ) {
